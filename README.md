    ...
    <style>
        * {
        ...
        .bg-animation {
            ...
            left: 0;
            z-index: 0;
            background: linear-gradient(135deg, #af8eee 0%, #764ba2 25%, #d88ce0 50%, #79b8ec 75%, #7aafd3 100%);
        ...
        .header h1 {
            font-size: 3.5rem;
            background: linear-gradient(135deg, #8d4bb6 0%, #7459c4 50%, #c5739c 100%);
...
<body>
        ...
        <div class="card">
            ...
            <div class="grid">
                <div class="form-group">
                    ...
                    <select id="genre">
                        <option value="sunoSoothing">AI Soothing</option>
            ...
            <div class="form-group" style="margin-top: 1rem;">
                ...
                <div class="voice-grid">
                    <div class="voice-card selected" onclick="toggleVoice('sunoSoft')" id="voice-sunoSoft">
                        <div class="voice-icon">🎭</div>
                        <div class="voice-name">Soft</div>
                        <div class="voice-desc">Gentle, expressive AI voice</div>
                    </div>
                    <div class="voice-card" onclick="toggleVoice('sunoWarm')" id="voice-sunoWarm">
                        <div class="voice-icon">🌟</div>
                        <div class="voice-name">Warm</div>
                        <div class="voice-desc">Rich, emotional AI vocals</div>
                    </div>
            ...
            <div class="card">
                ...
                <div class="team-grid">
                    <div class="team-member">
                        ...
                        <p style="color: #a78bfa; margin-bottom: 0.5rem;">AI Generalist</p>
                        <p style="font-size: 0.9rem; color: rgba(255, 255, 255, 0.7);">Web developer & build AI tools</p>
                    </div>
                    <div class="team-member">
                        <div class="team-avatar">👩‍🎨</div>
    ...
    <script>
        ...
        let currentLyrics = '';
        let isSinging = false;
    // default to a Suno-like soft voice for soothing output
    let selectedVoices = ['sunoSoft'];
        ...
        const voicePresets = {
            sunoSoft: {
                oscillator: {
                    type: 'custom',
                    partials: [1, 0.65, 0.45, 0.25, 0.15],
                    phase: 0,
                    harmonicity: 1.2
                },
                envelope: {
                ...
                effects: {
                    reverb: { decay: 2.8, wet: 0.45 },
                    delay: { time: '8n', feedback: 0.25, wet: 0.25 },
                    chorus: { frequency: 3.5, depth: 0.4, wet: 0.35 }
                }
            },
            sunoModern: {
                oscillator: {
                ...
                effects: {
                    ...
                    chorus: { frequency: 4, depth: 0.5, wet: 0.4 },
            ...
            sunoEmotional: {
                ...
                effects: {
                    ...
                    delay: { time: '8n.', feedback: 0.4, wet: 0.35 },
                    chorus: { frequency: 2.5, depth: 0.7, wet: 0.5 },
        ...
        function generateBotResponse(message) {
            ...
            if (lowerMsg.includes('genre') || lowerMsg.includes('style')) {
                ...
                return "Quantum mode uses real quantum random numbers from  quantum RNG! This creates truly unpredictable, unique melodies that are mathematically impossible to reproduce. It's perfect for experimental and one-of-a-kind compositions!";
        ...
        function initAudio() {
            if (!synth) {
                // Create main synth with effects
                ...
                synth.connect(mainReverb);
                synth.connect(mainDelay);
                synth.volume.value = -10;

                // Create genre-specific bass synths
                ...
                // Create multi-instrument drum kit
                drums = {
                ...
                drums.kick.volume.value = -6;
                drums.snare.volume.value = -8;
                drums.hihat.volume.value = -12;
        ...
        // Advanced melody templates inspired by Suno AI
        const advancedMelodyTemplates = {
            modern: {
                patterns: [
                    { notes: [0, 4, 7, 9, 7, 4], rhythm: [1, 0.5, 0.5, 1, 0.5, 0.5], velocity: [0.8, 0.6, 0.7, 0.8, 0.6, 0.7] },
                    { notes: [0, 7, 4, 2, 0, -1], rhythm: [1.5, 0.5, 0.5, 0.5, 0.5, 0.5], velocity: [0.9, 0.7, 0.6, 0.7, 0.6, 0.5] },
                    { notes: [4, 7, 11, 7, 4, 0], rhythm: [1, 1, 1, 0.5, 0.5, 1], velocity: [0.8, 0.9, 0.7, 0.6, 0.7, 0.8] }
                ],
                harmonies: [
                    [0, 4, 7],  // Major
                    [0, 3, 7],  // Minor
            ...
            emotional: {
                patterns: [
                    { notes: [0, 2, 4, 7, 9, 7], rhythm: [2, 1, 1, 1.5, 0.5, 1], velocity: [0.9, 0.7, 0.8, 0.9, 0.7, 0.6] },
                    { notes: [7, 4, 2, 0, -1, 0], rhythm: [1.5, 0.5, 1, 1, 0.5, 1.5], velocity: [0.8, 0.6, 0.7, 0.8, 0.7, 0.9] },
                    { notes: [4, 7, 9, 11, 12, 11], rhythm: [1, 1, 1, 1.5, 0.5, 1], velocity: [0.7, 0.8, 0.9, 0.8, 0.7, 0.6] }
                ],
                harmonies: [
                    [0, 3, 7, 10], // Minor 7th
                    [0, 4, 7, 11], // Major 7th
            ...
            epic: {
                patterns: [
                    { notes: [0, 4, 7, 12, 11, 7], rhythm: [2, 1, 1, 2, 1, 1], velocity: [1.0, 0.8, 0.9, 1.0, 0.9, 0.8] },
                    { notes: [7, 12, 11, 7, 4, 0], rhythm: [1.5, 0.5, 1, 1, 0.5, 1.5], velocity: [0.9, 1.0, 0.9, 0.8, 0.7, 0.9] },
                    { notes: [12, 11, 7, 4, 2, 0], rhythm: [1, 1, 1, 1, 0.5, 1.5], velocity: [1.0, 0.9, 0.8, 0.7, 0.8, 0.9] }
                ],
                harmonies: [
                    [0, 4, 7, 12],  // Major with octave
                    [0, 3, 7, 12],  // Minor with octave
                    [0, 5, 7, 12]   // Sus4 with octave
                ]
            },
            ambient: {
                patterns: [
                    { notes: [0, 4, 7, 11, 14, 11], rhythm: [3, 2, 2, 2, 1, 2], velocity: [0.6, 0.5, 0.7, 0.6, 0.5, 0.4] },
                    { notes: [7, 11, 14, 11, 7, 4], rhythm: [2, 2, 3, 2, 2, 1], velocity: [0.5, 0.6, 0.7, 0.6, 0.5, 0.4] },
                    { notes: [4, 7, 11, 14, 11, 7], rhythm: [2, 2, 2, 3, 2, 1], velocity: [0.4, 0.5, 0.6, 0.7, 0.5, 0.4] }
                ],
                harmonies: [
                    [0, 4, 7, 11, 14],  // Major 9th
                    [0, 3, 7, 10, 14],  // Minor 9th
                    [0, 4, 8, 11, 14]   // Augmented 9th
                ]
            }
        };
        ...
        const genrePresets = {
            ...
            experimental: { 
                scale: 'chromatic', 
                tempo: 140, 
                drumPattern: 'experimental',
                bassStyle: 'glitch',
                melodyOctave: 2,
                effects: { reverb: 0.6, delay: 0.4 }
            }
        };

        // Advanced Rhythm Patterns inspired by Suno AI
        function generateDrumPattern(type, duration) {
            const patterns = {
                fourOnFloor: {
                    ...
                    percussion: ['0:0:3', '0:1:3', '0:2:3', '0:3:3'],
                    cymbal: ['0:0:0', '0:2:0']
                },
                sparse: {
                    kick: ['0:0:0', '0:2:0'],
                    snare: ['0:1:2'],
                    hihat: ['0:0:2', '0:2:2'],
                    percussion: ['0:1:1', '0:3:1'],
                    cymbal: ['0:0:0']
                },
                gentle: {
                    kick: ['0:0:0', '0:3:0'],
                    snare: ['0:1:2', '0:3:2'],
                    hihat: ['0:0:2', '0:1:0', '0:2:2', '0:3:0'],
                    shaker: ['0:0:1', '0:1:1', '0:2:1', '0:3:1'],
                    chimes: ['0:0:0', '0:2:0']
                },
                sunoModern: {
                    kick: ['0:0:0', '0:1:2', '0:2:0', '0:3:2'],
                    snare: ['0:1:0', '0:3:0'],
                    hihat: ['0:0:1', '0:0:3', '0:1:1', '0:1:3', '0:2:1', '0:2:3', '0:3:1', '0:3:3'],
                    percussion: ['0:0:2', '0:1:2', '0:2:2', '0:3:2'],
                    cymbal: ['0:0:0', '0:2:0'],
                    shaker: ['0:0:2', '0:1:2', '0:2:2', '0:3:2']
                },
                sunoEpic: {
                    kick: ['0:0:0', '0:0:3', '0:2:0', '0:2:3'],
                    snare: ['0:1:0', '0:3:0'],
                    hihat: ['0:0:2', '0:1:2', '0:2:2', '0:3:2'],
                    percussion: ['0:0:1', '0:1:1', '0:2:1', '0:3:1'],
                    cymbal: ['0:0:0', '0:2:0'],
                    timpani: ['0:0:0', '0:2:0']
                },
                sunoAmbient: {
                ...
                hiphop: {
                    kick: ['0:0:0', '0:2:2', '0:3:0'],
                    snare: ['0:1:0', '0:3:0'],
                    hihat: ['0:0:2', '0:1:2', '0:2:0', '0:3:2']
                },
                rock: {
                    kick: ['0:0:0', '0:1:2', '0:2:0', '0:3:2'],
                    snare: ['0:1:0', '0:3:0'],
                ...
                experimental: {
                    kick: ['0:0:0', '0:1:3', '0:2:2', '0:3:1'],
                    snare: ['0:0:2', '0:2:0', '0:3:3'],
                    hihat: ['0:0:1', '0:1:2', '0:2:3', '0:3:0']
                }
            };
            let selectedPattern = patterns[type] || patterns.fourOnFloor;
            let fullPattern = [];
            // limit bars for faster generation and reasonable loop sizes
            const bars = Math.min(Math.ceil(duration / 2), 8);
            
            for (let bar = 0; bar < bars; bar++) {
                // Add kick drum pattern
                selectedPattern.kick.forEach(time => {
                ...
                // Add snare pattern
                selectedPattern.snare.forEach(time => {
                    const [b, beat, sub] = time.split(':').map(Number);
                    fullPattern.push({
                        time: `${bar}:${beat}:${sub}`,
                        note: 'D2',
                ...
                // Add hi-hat pattern
                selectedPattern.hihat.forEach(time => {
                    const [b, beat, sub] = time.split(':').map(Number);
                    fullPattern.push({
                        time: `${bar}:${beat}:${sub}`,
                        note: 'F#2',
                ...
                // Add other optional percussive elements if present (map to simple instruments)
                const optionalInstruments = ['percussion', 'shaker', 'cymbal', 'chimes', 'timpani', 'bells', 'pad'];
                optionalInstruments.forEach(instr => {
                    if (selectedPattern[instr]) {
                        selectedPattern[instr].forEach(time => {
                            const [b, beat, sub] = time.split(':').map(Number);
                            fullPattern.push({
                                time: `${bar}:${beat}:${sub}`,
                                note: instr === 'percussion' ? 'C2' : (instr === 'chimes' || instr === 'bells' ? 'E4' : 'C1'),
                                velocity: instr === 'cymbal' || instr === 'chimes' ? 0.45 : 0.55,
                                instrument: instr
                            });
                        });
                    }
        ...
        // Generate Soothing Melody
        function generateSoothingMelody(scale, baseNote, duration, mood) {
            const melodyType = mood === 'calm' ? 'calm' : 
                             mood === 'romantic' ? 'gentle' : 'dreamy';
            const template = soothingMelodyTemplates[melodyType];
            const selectedPattern = template[Math.floor(Math.random() * template.length)];
            
            const melody = [];
            let currentTime = 0;
            
            const bars = Math.min(Math.max(1, Math.ceil(duration / 4)), 8);
            for (let i = 0; i < bars; i++) {
                for (let j = 0; j < selectedPattern.pattern.length; j++) {
                    const noteIndex = selectedPattern.pattern[j] >= 0 ? 
                        selectedPattern.pattern[j] % scale.length : 
                        scale.length - 1;
                    const note = baseNote + scale[noteIndex];
                    melody.push({
                        note: Tone.Frequency(note, 'midi').toNote(),
                        time: currentTime,
                        duration: selectedPattern.rhythm[j] + 'n',
                        velocity: 0.6 + Math.random() * 0.2
                    });
                    currentTime += selectedPattern.rhythm[j] * 0.5;
                }
            }
            return melody;
        }

        // Template-based melody generator (fast) using seeded RNG
        function generateTemplateMelody(scale, baseNote, duration, genreSettings, mood, rng) {
            // choose melody style from genre settings or fallback
            const style = genreSettings.melodyStyle || 'modern';
            const templateBank = advancedMelodyTemplates[style] || advancedMelodyTemplates.modern;
            const patternObj = templateBank.patterns[Math.floor(rng() * templateBank.patterns.length)];

            const melody = [];
            let currentTime = 0;
            // limit bars to keep generation fast
            const bars = Math.min(Math.max(1, Math.ceil(duration / 4)), 8);

            for (let b = 0; b < bars; b++) {
                for (let i = 0; i < patternObj.notes.length; i++) {
                    const noteOffset = patternObj.notes[i];
                    // map noteOffset into scale
                    const scaleIndex = ((noteOffset % scale.length) + scale.length) % scale.length;
                    const octaveShift = Math.floor(noteOffset / scale.length);
                    const midi = baseNote + scale[scaleIndex] + octaveShift * 12;
                    const dur = (patternObj.rhythm && patternObj.rhythm[i]) ? patternObj.rhythm[i] : 1;
                    const velArr = patternObj.velocity || [];
                    const velocity = (velArr[i] !== undefined) ? velArr[i] : (0.6 + rng() * 0.3);
                    ...
                    currentTime += dur * 0.5; // scale timing down for denser but pleasant phrasing
                    // stop if exceeds requested duration
        ...
        // Generate Bass Line
        function generateBassLine(melody, scale, baseNote, style) {
            const bassNotes = [];
            let bassPattern, noteDuration, velocityRange;

            switch(style) {
                case 'synth':
                    bassPattern = [0, 4, 2, 6];
                    noteDuration = '4n';
                    velocityRange = { min: 0.6, max: 0.8 };
                    break;
                case 'pad':
                    bassPattern = [0, 0, 4, 4];
                    noteDuration = '2n';
                ...
                case 'orchestral':
                    bassPattern = [0, 2, 4, 3];
                    noteDuration = '2n';
                    velocityRange = { min: 0.5, max: 0.7 };
                    break;
                case 'acoustic':
                    bassPattern = [0, 3, 5, 7];
                    noteDuration = '4n';
                    velocityRange = { min: 0.7, max: 0.9 };
                    break;
                case '808':
                    bassPattern = [0, 0, 7, 0];
                    noteDuration = '8n';
                    velocityRange = { min: 0.8, max: 1.0 };
                    break;
                case 'distorted':
                    bassPattern = [0, 5, 7, 3];
                    noteDuration = '4n';
            ...
            for (let i = 0; i < melody.length; i += 4) {
                ...
                const velocity = velocityRange.min + Math.random() * (velocityRange.max - velocityRange.min);
        ...
        async function generateMelody(genre, mood, duration) {
            ...
            const genreSettings = genrePresets[genre] || genrePresets.electronic;
            const scale = scales[genreSettings.scale] || scales.major;
            const baseNote = 60;
            const numNotes = Math.max(8, Math.floor(duration / 0.5));
            ...
            // Create a seed once (fast). If quantum is enabled, use a single quantum call to seed RNG
            let seed;
            if (useQuantum) {
                const q = await getQuantumRandomNumber();
                seed = Math.floor(q * 2147483647) ^ (Date.now() & 0x7fffffff);
            } else {
                const arr = new Uint32Array(1);
                crypto.getRandomValues(arr);
                seed = arr[0] ^ (Date.now() & 0x7fffffff);
            }

            const rng = mulberry32(seed);

            // If the genre is one of the Suno-style presets (or has a melodyStyle), use the fast template generator
            if ((genre && genre.indexOf('suno') === 0) || genreSettings.melodyStyle) {
                return generateTemplateMelody(scale, baseNote, duration, genreSettings, mood, rng);
            }

            // Regular (but faster) melody generation using seeded RNG (no per-note async calls)
            const melody = [];
            const rhythmPattern = [0.5, 0.5, 1, 0.5, 0.5, 1];
            let currentTime = 0;

            for (let i = 0; i < numNotes; i++) {
                const scaleIndex = Math.floor(rng() * scale.length);
                const octaveShift = genreSettings.melodyOctave + (Math.floor(rng() * 3) - 1);
                const midi = baseNote + scale[scaleIndex] + (octaveShift * 12);

                let velocity;
                switch (mood) {
                    case 'uplifting': velocity = 0.75 + rng() * 0.2; break;
                    case 'calm': velocity = 0.45 + rng() * 0.2; break;
                    case 'energetic': velocity = 0.85 + rng() * 0.1; break;
                    case 'dark': velocity = 0.45 + rng() * 0.25; break;
                    case 'romantic': velocity = 0.55 + rng() * 0.2; break;
                    case 'epic': velocity = 0.85 + rng() * 0.12; break;
                    default: velocity = 0.55 + rng() * 0.25;
                }

                const rhythmIndex = i % rhythmPattern.length;
                const dur = rhythmPattern[rhythmIndex];
                ...
                currentTime += dur * 0.45; // slightly denser phrasing for smoother flow
                if (currentTime > duration) break;
            }
        ...
        async function playTrack(track) {
            ...
            if (track.drums) {
                drumSequence = new Tone.Part((time, note) => {
                    switch(note.instrument) {
                        case 'kick':
                            drums.kick.triggerAttackRelease(note.note, '8n', time, note.velocity);
                            break;
                        case 'snare':
                            drums.snare.triggerAttackRelease('8n', time, note.velocity);
                            break;
                        case 'hihat':
                            drums.hihat.triggerAttackRelease('32n', time, note.velocity * 0.8);
                            break;
                        case 'shaker':
                        case 'percussion':
                            // map lighter percussive sounds to hi-hat for now
                            drums.hihat.triggerAttackRelease('64n', time, (note.velocity || 0.5) * 0.6);
                            break;
                        case 'cymbal':
                        case 'chimes':
                        case 'bells':
                        case 'timpani':
                        case 'pad':
                            // map melodic/percussive accents to snare/noise synth with low velocity
                            drums.snare.triggerAttackRelease('8n', time, (note.velocity || 0.6) * 0.45);
                            break;
                        default:
                            // fallback to kick
                            drums.kick.triggerAttackRelease(note.note || 'C1', '8n', time, note.velocity || 0.6);
                    }
                }, track.drums);
            ...
            if (track.lyrics && track.lyrics.type === 'recording') {
                ...
                const voiceSynths = selectedVoices.map(voice => {
                    ...
                    const osc = preset.oscillator || { type: 'sine' };
                    const env = preset.envelope || { attack: 0.1, decay: 0.2, sustain: 0.6, release: 0.8 };
                    ...
                    // Reverb (support both simple and structured presets)
                    const reverbWet = (preset.effects && preset.effects.reverb && preset.effects.reverb.wet) || preset.reverb || 0.3;
                    const reverbDecay = (preset.effects && preset.effects.reverb && preset.effects.reverb.decay) || 2.5;
                    const reverb = new Tone.Reverb({ decay: reverbDecay, wet: reverbWet }).toDestination();
                    synth.connect(reverb);

                    // Optional delay
                    if (preset.effects && preset.effects.delay) {
                        const d = preset.effects.delay;
                        const delay = new Tone.FeedbackDelay({ delayTime: d.time || '8n', feedback: d.feedback || 0.25, wet: d.wet || 0.2 }).toDestination();
                        synth.connect(delay);
                    }

                    // Optional chorus
                    if (preset.effects && preset.effects.chorus) {
                        const c = preset.effects.chorus;
                        try {
                            const chorus = new Tone.Chorus(c.frequency || 4, c.depth || 0.4, c.wet || 0.2).toDestination();
                            synth.connect(chorus);
                        } catch (e) {
                            // some Tone.js builds may not include Chorus; ignore if unavailable
                        }
                    }
        ...
        document.getElementById('generateBtn').addEventListener('click', async function() {
            ...
            setTimeout(async () => {
                ...
                const preset = genrePresets[genre] || genrePresets.electronic;
                const scale = scales[preset.scale] || scales.major;
                const bassLine = generateBassLine(melody, scale, 60, preset.bassStyle || 'synth');
                const drums = generateDrumPattern(preset.drumPattern || 'fourOnFloor', duration);
